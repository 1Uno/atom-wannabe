...
class Frames {
   ...
   */
  static _createElement (editor, frames) {
    let text = '';
  ...
  static _createMarker (editor, line) {
    return editor.markBufferPosition ([line - 1, 0], {invalidate: 'never'});
  }
  ...
  setFrames (editor, frames) {
    ...
    Object.keys (this._frames).forEach ((line) => {
      const element = Frames._createElement (editor, this._frames[line]);
