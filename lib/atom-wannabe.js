...
class Frames {
   ...
   */
  static _createDump (editor, frame) {
    let text = '';
  ...
  static _createCheckFrame (editor, frame) {
  ...
  setFrame (editor, frame) {
    const {line}      = frame;
    const element     = Frames._createDump (editor, frame);
    ...
    editor.gutterWithName ('markerTest').decorateMarker (markerTest, {
      type:  'gutter',
      item:  Frames._createCheckFrame (editor, frame),
  ...
  setFrames (editor, frames) {
    ...
    Object.keys (frames).forEach ((line) => this.setFrame (editor, frames[line]));
  }
  ...
  setTests (editor, tests) {
    ...
    tests.forEach ((test) => this.setTest (editor, test));
  }
}
...
export default {
  ...
  _runWannabe2: watt (function * (editor, filePath, fileText, extractFrom, framesObj) {
    const wannabe = new Wannabe (filePath, fileText, 'it', extractFrom);
    const runner = yield wannabe.runner ();

    runner.on ('frame', (frame) => {
      framesObj.setFrame (editor, frame);
    });

    runner.on ('test', (test) => {
      framesObj.setTest (editor, test);
    });
  ...
  _runWannabe: watt (function * (editor, framesObj) {
    ...
    const done = yield this._runWannabe2 (editor, filePath, fileText, line, framesObj);
    if (!done) {
      yield this._runWannabe2 (editor, filePath, fileText, /.*/, framesObj);
    }
  }),
