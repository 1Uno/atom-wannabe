...
class MarkRegistry {
  ...
  invalidMarks () {
    this._invalidMarks = this._validMarks;
    this._validMarks = {
      test: {},
      frame: {}
    };
  ...
  createTestMark (test) {
    ...
    if (this._invalidMarks.test[test.line]) {
      this._invalidMarks.test[test.line].dispose (); // FIXME: reuse instead of dispose
  ...
  createFrameMark (frame) {
    ...
    if (this._invalidMarks.frame[frame.line]) {
      this._invalidMarks.frame[frame.line].dispose (); // FIXME: reuse instead of dispose
