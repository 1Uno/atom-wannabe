...
class MarkRegistry {
  ...
  checkInvalids (tests) {
    const testLines = tests.map ((test) => test.line);

    Object
      .keys (this._invalidMarks.frame)
      .filter ((line) => testLines.indexOf (this._invalidMarks.frame[line].data.test.line) === -1)
      .forEach ((line) => {
    ...
    Object
      ...
      .filter ((line) => testLines.indexOf (this._invalidMarks.test[line].line) === -1)
      .forEach ((line) => {
