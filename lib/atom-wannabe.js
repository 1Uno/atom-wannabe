...
class Frames {
  ...
  setFrames (editor, frames) {
    ...
    this._frames = frames;
    Object.keys (this._frames).forEach ((line) => this.setFrame (editor, line));
  }
...
export default {
  ...
  _runWannabe: watt (function * (editor, framesObj) {
    ...
    let wannabe = new Wannabe (filePath, fileText, 'it', line);
    ...
    if (frames && Object.keys (frames).length) {
      ...
      wannabe = new Wannabe (filePath, fileText, 'it', /.*/);
      const {frames, tests} = yield wannabe.run ();
      if (frames && Object.keys (frames).length) {
        framesObj.setFrames (editor, frames);
        framesObj.setTests (editor, tests);
