...
class MarkRegistry {
  constructor (atomWannabe, editor) {
    this._editor = editor;
  ...
  dispose () {
    Object.keys (this._validMarks.frame).forEach (line =>
      this._validMarks.frame[line].dispose ()
    );
    Object.keys (this._validMarks.test).forEach (line =>
      this._validMarks.test[line].dispose ()
    );
  ...
  invalidDispose () {
    Object.keys (this._invalidMarks.frame).forEach (line =>
      this._invalidMarks.frame[line].dispose ()
    );
    Object.keys (this._invalidMarks.test).forEach (line =>
      this._invalidMarks.test[line].dispose ()
    );
  ...
  checkInvalids (tests) {
    ...
    Object.keys (this._invalidMarks.frame)
      .filter (
        line =>
          !testLines ||
          testLines.indexOf (
            this._invalidMarks.frame[line].data[0].test.line
          ) === -1
      )
      .forEach (line => {
        this._validMarks.frame[line] = this._invalidMarks.frame[line];
        this._validMarks.frame[line].unconfirm ();
        delete this._invalidMarks.frame[line];
      });

    Object.keys (this._invalidMarks.test)
      .filter (line => !testLines || testLines.indexOf (line) === -1)
      .forEach (line => {
        this._validMarks.test[line] = this._invalidMarks.test[line];
        this._validMarks.test[line].unconfirm ();
        delete this._invalidMarks.test[line];
      });
  }

  checkIfExists (frame) {
    return Object.keys (this._validMarks.frame).some (line =>
      this._validMarks.frame[line].checkIfExists (frame)
    );
  }
  ...
  createTestMark (test) {
    ...
    if (this._validMarks.test[test.line]) {
      ...
      const mark = new TestMark (this._atomWannabe, this._editor, test);
