...
class TestMark {
  constructor (editor, test) {
    ...
    this._mark = this._editor.markBufferPosition ([parseInt (test.line) - 1, 999], {invalidate: 'surround'});

    this._event = this._editor.onDidChangeScrollTop (() => {
    ...
    this._decorMark = this._editor.decorateMarker (this._mark, {
      type:  'overlay',
      item:   createBadge (this._editor, test),
      class: this._visible ? 'atom-wannabe-marker-test' : 'atom-wannabe-marker-test-hide'
    });
  ...
  unconfirm () {
    ...
    this._decorGutter = this._editor.gutterWithName ('markerTest').decorateMarker (this._mark, props);
