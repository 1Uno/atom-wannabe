...
const createConsole = frame => {
  const element = document.createElement ('div');
  element.className = 'atom-wannabe-marker-frame-console-div';
  element.textContent = `${frame.payload.console.type}:${frame.payload.console.text}`;
  return element;
};

const createDump = (editor, frames) => {
  let divs = [];
...
const createCheckFrame = (editor, frames) => {
